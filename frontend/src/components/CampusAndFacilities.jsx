...
// // Bubble Background Component (same as About Anish page)
...
// =========================================================
// === NEW Background Animation Components ===
// =========================================================

// 1. Wavy Background (For Hero Section - Light BG)
const WavyBackground = () => (
    <div className={`${styles.backgroundAnimation} ${styles.wavyBackground}`}>
...
// 2. Flowing Dots Background (For Facilities Section - Light BG)
...
// 3. Bubble Background (Kept for Banner Section - Dark BG)
const BubbleBackground = () => (
    <div className="absolute inset-0 overflow-hidden opacity-50">
        {/* CSS is injected via dangerouslySetInnerHTML in the original code, keeping it for consistency */}
        <style dangerouslySetInnerHTML={{__html: `
            @keyframes float {
                0% { transform: translateY(0) rotate(0deg); opacity: 1; }
                100% { transform: translateY(-1000px) rotate(720deg); opacity: 0; }
...
// =========================================================
// === END Background Animation Components ===
// =========================================================

// Helper component for Facility Icons (Kept as is)
const FacilityIcon = ({ icon }) => (
    <motion.div 
        className={styles.facilityIconWrapper}
        initial={{ opacity: 0, scale: 0 }}
...
// Helper component for Gallery Items (Kept as is)
const GalleryItem = ({ item, index }) => (
  <MotionDiv 
    key={index} 
    className={styles.galleryItem}
...
function CampusAndFacilities() {
  ...
  return (
    ...
    >
      ...
      {/* ===== SECTION 1: HERO SECTION - White Background with WAVY ANIMATION ===== */}
      <MotionSection 
        className={styles.heroSection}
        variants={containerVariants} 
      >
      ...
      {/* --- Section 2: CAMPUS GALLERY - Light Background Color (No New Animation Here) --- */}
      <MotionSection 
        className={`${styles.section} ${styles.lightBg}`} 
        initial="hidden"
        whileInView="visible"
      ...
      {/* --- Section 3: WORLD-CLASS FACILITIES - White Background Color with DOTS ANIMATION --- */}
      <MotionSection 
        className={`${styles.section} ${styles.relativePosition}`} /* Added relativePosition for background */
        initial="hidden"
        whileInView="visible"
        viewport={{ once: true, amount: 0.1 }}
        variants={containerVariants}
      ...
      {/* ===== SECTION 4: LIVING & LEARNING - Dark Primary BG Banner (Bubble Animation Kept) ===== */}
      <MotionSection 
        initial="hidden"
        whileInView="visible"
        viewport={{ once: true, amount: 0.1 }}
      ...
      >
        {/* Banner (Dark BG) */}
        <div className={styles.banner}>
          {/* Bubble Background is kept here as requested */}
          <BubbleBackground /> 
          <div className={styles.container}>
            <MotionDiv variants={fadeInUp}>
              <h2 className={styles.bannerTitle}>Living & Learning Together</h2>
